...
(use-package ansible
  :ensure t
  :if (pfp-command-exists-p 'ansible)
  :config
  (progn
...
(use-package x509-mode
  :ensure t
  :if (pfp-command-exists-p 'openssl))
...
(use-package keystore-mode
  :ensure t
  :if (pfp-command-exists-p 'keytool))
#+END_SRC
...
(defun pfp/xrandr-desk-3-window ()
  (interactive)
...
(defun pfp/xrandr-desk-2-window ()
  (interactive)
